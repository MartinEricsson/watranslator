...
export default async function (debug = false) {
	try {
		...
		const { wasmBuffer } = await readTestData(
			"imported-globals/imported-globals.wat",
			debug,
		);
		...
		if (typeof WebAssembly !== "undefined") {
			if (debug)
				console.log("Testing WebAssembly instantiation with imports...");

			// Test accessing the imported global value
			const importedY = instance.exports.getY();
			if (importedY !== 123) {
				console.error(
					`Imported global value mismatch: expected 123, got ${importedY}`,
				);
				return false;
			}
			...
			if (localX !== 42) {
				console.error(
					`Local global value mismatch: expected 42, got ${localX}`,
				);
				return false;
			}
