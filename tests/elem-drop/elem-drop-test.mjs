...
async function testElemDrop(debug = false) {
	try {
		...
		assert(
			ast[0].elements && ast[0].elements.length === 2,
			"Should have two element segments",
		);
		...
		assert.strictEqual(
			getFunc(0)(),
			42,
			"After drop: Function at index 0 should still return 42",
		);
		assert.strictEqual(
			getFunc(1)(),
			43,
			"After drop: Function at index 1 should still return 43",
		);
