...
async function testF64Load(debug = false) {
	...
	const { wasmBuffer } = await readTestData(
		"f64-math-and-ops/f64-load-test.wat",
		debug,
	);
	...
	if (typeof instance.exports.load_f64 === "function") {
		...
		for (const test of testCases) {
			...
			console.assert(
				resultRes,
				`❌ load_f64(${test.addr}) should return ${test.expected}, got ${result}`,
			);
			if (debug)
				console.log(
					`load_f64(${test.addr}) = ${result}, expected: ${test.expected} - ${resultRes ? "✅" : "❌"}`,
				);
		}
	...
	console.error(
		"❌ f64.load test failed: load_f64 function not found in exports",
	);
