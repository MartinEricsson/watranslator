...
async function testF64Store(debug = false) {
	try {
		...
		const { wasmBuffer } = await readTestData(
			"f64-math-and-ops/f64-store-test.wat",
			debug,
		);
		...
		for (const test of testCases) {
			...
			console.log(
				`Store/Load at address ${test.addr}: stored ${test.value}, loaded ${result} - ${equalityResult ? "✅" : "❌"}`,
			);
		}
		...
		for (const test of testCases) {
			...
			console.log(
				`store_and_load at address ${test.addr}: value ${test.value}, result ${result} - ${equalityResult ? "✅" : "❌"}`,
			);
		}
		...
		for (let i = 0; i < testCases.length; i++) {
			...
			console.log(
				`Raw memory at float index ${addr}: expected ${test.value}, found ${memValue} - ${equalityResult ? "✅" : "❌"}`,
			);
		}
