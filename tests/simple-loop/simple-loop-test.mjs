		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.countToN === "function") {
				...
				for (const test of testCases) {
					...
					console.log(
						`countToN(${test.n}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (testsPassed) {
					...
					console.error(
						"❌ Loop test failed: countToN function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Loop test failed: countToN function not found in exports",
				);
				return false;
			}
