		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.divide_signed === "function") {
				...
				for (const test of testCasesSigned) {
					...
					console.log(
						`divide_signed(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (testsPassed) {
					console.log(
						"✅ Signed division test: divide_signed function works correctly!",
					);
				} else {
					console.error(
						"❌ Signed division test failed: divide_signed function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Signed division test failed: divide_signed function not found in exports",
				);
				return false;
			}
			...
			if (typeof instance.exports.divide_unsigned === "function") {
				...
				for (const test of testCasesUnsigned) {
					...
					const passed = result === test.expected;
					console.log(
						`divide_unsigned(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					console.log(
						"✅ Unsigned division test: divide_unsigned function works correctly!",
					);
				} else {
					console.error(
						"❌ Unsigned division test failed: divide_unsigned function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Unsigned division test failed: divide_unsigned function not found in exports",
				);
				return false;
			}
