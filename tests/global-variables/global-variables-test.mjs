...
async function testGlobalVariables(debug = false) {
	try {
		const { wasmBuffer, ast } = await readTestData(
			"global-variables/global-variables.wat",
			debug,
		);
		...
		if (typeof WebAssembly !== "undefined") {
			...
			assert.strictEqual(
				answerGlobal,
				42,
				"Exported global value should be 42",
			);
			...
			assert.ok(
				Math.abs(fahrenheit - 72.5) < 0.01,
				"Fahrenheit conversion should be approximately 72.5°F",
			);
			...
			assert.ok(
				Math.abs(newFahrenheit - 86) < 0.01,
				"Updated temperature conversion should be 86°F",
			);

			// Check the exported global value was updated
			const tempGlobal = exports.temperature.value;
			assert.ok(
				Math.abs(tempGlobal - 30) < 0.01,
				"Exported temperature global should be updated to 30",
			);
