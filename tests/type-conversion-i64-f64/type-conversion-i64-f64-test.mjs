...
async function testTypeConversionI64ToF64(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"type-conversion-i64-f64/type-conversion-i64-f64.wat",
			debug,
		);
		...
		const tests = [
			...
			{
				name: "convert_i64_s_to_f64",
				expected: Number(9223372036854775807n), // large i64, precision loss acceptable, handled via relative error
				func: "convert_i64_s_to_f64",
				params: [9223372036854775807n],
			}, // Max i64
			{
				name: "convert_i64_s_to_f64",
				expected: Number(-9223372036854775808n), // min i64
				func: "convert_i64_s_to_f64",
			...
			{
				name: "convert_i64_u_to_f64",
				expected: Number(9223372036854775807n),
				func: "convert_i64_u_to_f64",
				params: [9223372036854775807n],
			}, // Max i64
			{
				name: "convert_i64_u_to_f64",
				expected: Number(18446744073709551615n), // max u64
				func: "convert_i64_u_to_f64",
				params: [-1n],
			}, // Unsigned interpretation of -1
			{
				name: "convert_i64_u_to_f64",
				expected: Number(10000000000000000000n),
				func: "convert_i64_u_to_f64",
				params: [10000000000000000000n],
			}, // Large value > 2^63
			...
			{
				name: "test_min_i64",
				expected: Number(-9223372036854775808n),
				func: "test_min_i64",
				params: [],
			},
			{
				name: "test_unsigned_large",
				expected: Number(18446744073709551615n),
				func: "test_unsigned_large",
				params: [],
			}, // 2^64-1
			{
				name: "test_unsigned_medium",
				expected: Number(9223372036854775807n),
				func: "test_unsigned_medium",
				params: [],
			}, // 2^63-1
		];
		...
		for (const { name, expected, func, params } of tests) {
			...
			if (debug || !isEqual) {
				console.log(
					`${name} with params [${params}]: result=${result}, expected=${expected}, ${isEqual ? "✅" : "❌"}`,
				);
			}
		}
