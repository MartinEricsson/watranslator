...
async function testSIMDI64x2Compare(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-i64x2-compare/simd-i64x2-compare.wat",
			debug,
		);
		...
		if (eqTrueResult !== -1) {
			throw new Error(
				`i64x2.eq true test failed: expected -1, got ${eqTrueResult}`,
			);
		}
		...
		if (eqFalseResult !== 0) {
			throw new Error(
				`i64x2.eq false test failed: expected 0, got ${eqFalseResult}`,
			);
		}
		...
		if (neTrueResult !== -1) {
			throw new Error(
				`i64x2.ne true test failed: expected -1, got ${neTrueResult}`,
			);
		}
		...
		if (neFalseResult !== 0) {
			throw new Error(
				`i64x2.ne false test failed: expected 0, got ${neFalseResult}`,
			);
		}
		...
		if (ltTrueResult !== -1) {
			throw new Error(
				`i64x2.lt_s true test failed: expected -1, got ${ltTrueResult}`,
			);
		}
		...
		if (ltFalseResult !== 0) {
			throw new Error(
				`i64x2.lt_s false test failed: expected 0, got ${ltFalseResult}`,
			);
		}
		...
		if (gtTrueResult !== -1) {
			throw new Error(
				`i64x2.gt_s true test failed: expected -1, got ${gtTrueResult}`,
			);
		}
		...
		if (gtFalseResult !== 0) {
			throw new Error(
				`i64x2.gt_s false test failed: expected 0, got ${gtFalseResult}`,
			);
		}
		...
		if (leTrueLtResult !== -1) {
			throw new Error(
				`i64x2.le_s true (lt) test failed: expected -1, got ${leTrueLtResult}`,
			);
		}
		...
		if (leTrueEqResult !== -1) {
			throw new Error(
				`i64x2.le_s true (eq) test failed: expected -1, got ${leTrueEqResult}`,
			);
		}
		...
		if (leFalseResult !== 0) {
			throw new Error(
				`i64x2.le_s false test failed: expected 0, got ${leFalseResult}`,
			);
		}
		...
		if (geTrueGtResult !== -1) {
			throw new Error(
				`i64x2.ge_s true (gt) test failed: expected -1, got ${geTrueGtResult}`,
			);
		}
		...
		if (geTrueEqResult !== -1) {
			throw new Error(
				`i64x2.ge_s true (eq) test failed: expected -1, got ${geTrueEqResult}`,
			);
		}
		...
		if (geFalseResult !== 0) {
			throw new Error(
				`i64x2.ge_s false test failed: expected 0, got ${geFalseResult}`,
			);
		}
		...
		if (signedResult !== -1n) {
			throw new Error(
				`i64x2 signed comparison test failed: expected -1n, got ${signedResult}`,
			);
		}
