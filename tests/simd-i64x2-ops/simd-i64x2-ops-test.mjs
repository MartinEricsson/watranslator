...
async function testSIMDI64x2Ops(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-i64x2-ops/simd-i64x2-ops.wat",
			debug,
		);
		...
		if (allTrueTrueResult !== 1) {
			throw new Error(
				`i64x2.all_true true test failed: expected 1, got ${allTrueTrueResult}`,
			);
		}
		...
		if (allTrueFalseResult !== 0) {
			throw new Error(
				`i64x2.all_true false test failed: expected 0, got ${allTrueFalseResult}`,
			);
		}
		...
		if (bitmaskResult !== 1) {
			throw new Error(
				`i64x2.bitmask test failed: expected 1, got ${bitmaskResult}`,
			);
		}
		...
		if (extendLowSResult !== -1n) {
			throw new Error(
				`i64x2.extend_low_i32x4_s test failed: expected -1n, got ${extendLowSResult}`,
			);
		}
		...
		log(
			`i64x2.extend_high_i32x4_s test: got ${extendHighSResult}, expected -3`,
		);
		if (extendHighSResult !== -3n) {
			throw new Error(
				`i64x2.extend_high_i32x4_s test failed: expected -3n, got ${extendHighSResult}`,
			);
		}
		...
		log(
			`i64x2.extend_low_i32x4_u test: got ${extendLowUResult}, expected 4294967295`,
		);
		if (extendLowUResult !== 4294967295n) {
			throw new Error(
				`i64x2.extend_low_i32x4_u test failed: expected 4294967295n, got ${extendLowUResult}`,
			);
		}
		...
		log(
			`i64x2.extend_high_i32x4_u test: got ${extendHighUResult}, expected 4294967293`,
		);
		if (extendHighUResult !== 4294967293n) {
			throw new Error(
				`i64x2.extend_high_i32x4_u test failed: expected 4294967293n, got ${extendHighUResult}`,
			);
		}
		...
		if (shrSResult !== -2n) {
			throw new Error(
				`i64x2.shr_s test failed: expected -2n, got ${shrSResult}`,
			);
		}
		...
		if (shrUResult !== expectedShrU) {
			throw new Error(
				`i64x2.shr_u test failed: expected ${expectedShrU}, got ${shrUResult}`,
			);
		}
		...
		if (extmulLowSResult !== 8n) {
			throw new Error(
				`i64x2.extmul_low_i32x4_s test failed: expected 8n, got ${extmulLowSResult}`,
			);
		}
		...
		log(
			`i64x2.extmul_high_i32x4_s test: got ${extmulHighSResult}, expected 24`,
		);
		if (extmulHighSResult !== 24n) {
			throw new Error(
				`i64x2.extmul_high_i32x4_s test failed: expected 24n, got ${extmulHighSResult}`,
			);
		}
		...
		if (extmulLowUResult !== 8n) {
			throw new Error(
				`i64x2.extmul_low_i32x4_u test failed: expected 8n, got ${extmulLowUResult}`,
			);
		}
		...
		log(
			`i64x2.extmul_high_i32x4_u test: got ${extmulHighUResult}, expected 24`,
		);
		if (extmulHighUResult !== 24n) {
			throw new Error(
				`i64x2.extmul_high_i32x4_u test failed: expected 24n, got ${extmulHighUResult}`,
			);
		}
