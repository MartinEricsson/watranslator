...
async function testSIMDi32x4Ops(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-i32x4-ops/simd-i32x4-ops.wat",
			debug,
		);
		...
		if (absResult <= 0) {
			throw new Error(
				`i32x4.abs test failed: expected positive value, got ${absResult}`,
			);
		}
		...
		const extaddPairwiseSResult =
			instance.exports.i32x4_extadd_pairwise_i16x8_s_test();
		log(`i32x4_extadd_pairwise_i16x8_s_test: ${extaddPairwiseSResult}`);

		const extaddPairwiseUResult =
			instance.exports.i32x4_extadd_pairwise_i16x8_u_test();
