...
async function testSIMDVectorOps(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-vector-ops/simd-vector-ops.wat",
			debug,
		);
		...
		if (andnotResult !== 1) {
			throw new Error(
				`v128.andnot test failed: expected 1, got ${andnotResult}`,
			);
		}
		...
		if (bitselectResult !== 1) {
			throw new Error(
				`v128.bitselect test failed: expected 1, got ${bitselectResult}`,
			);
		}
		...
		if (anyTrueResult !== 1) {
			throw new Error(
				`v128.any_true (true case) test failed: expected 1, got ${anyTrueResult}`,
			);
		}
		...
		if (anyFalseResult !== 0) {
			throw new Error(
				`v128.any_true (false case) test failed: expected 0, got ${anyFalseResult}`,
			);
		}
