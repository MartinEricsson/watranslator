...
async function testMemoryInit(debug = false) {
	try {
		...
		console.log(
			"✅ Initial memory contents are correct (active segment initialized properly)",
		);
		...
		for (let i = 0; i < 5; i++) {
			...
			if (debug)
				console.log(
					`Byte at offset ${100 + i} after init: 0x${actual.toString(16).padStart(2, "0")} ✓`,
				);
		}
		console.log(
		...
		for (let i = 0; i < 5; i++) {
			...
			if (debug)
				console.log(
					`Byte at offset ${200 + i} after partial init: 0x${actual.toString(16).padStart(2, "0")} ✓`,
				);
		}
		console.log(
		...
		for (let i = 0; i < 5; i++) {
			...
			if (debug)
				console.log(
					`readByte(${100 + i}): 0x${actual.toString(16).padStart(2, "0")} ✓`,
				);
		}
		console.log(
			"✅ readByte function correctly reads memory after initialization",
		);
