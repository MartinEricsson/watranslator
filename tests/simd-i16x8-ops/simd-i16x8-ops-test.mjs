import { dumpWASM, logger, readTestData } from "../test-utils.mjs";
...
async function testSIMDi16x8Ops(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-i16x8-ops/simd-i16x8-ops.wat",
			debug,
		);
		...
		if (absResult <= 0) {
			throw new Error(
				`i16x8.abs test failed: expected positive value, got ${absResult}`,
			);
		}
		...
		if (negResult >= 0) {
			throw new Error(
				`i16x8.neg test failed: expected negative value, got ${negResult}`,
			);
		}
		...
		if (allTrueResult !== 1) {
			throw new Error(
				`i16x8.all_true test failed: expected 1, got ${allTrueResult}`,
			);
		}
...
function testNumericResult(instance, testName, log) {
	...
	if (typeof result !== "number") {
		throw new Error(
			`${testName} failed: expected a number, got ${typeof result}`,
		);
	}
}
