...
async function testTableCopy(debug = false) {
	try {
		...
		assert(
			ast[0].elements && ast[0].elements.length === 1,
			"Should have one element segment",
		);
		...
		assert.strictEqual(
			getFunc(0)(),
			42,
			"Initial: Function at index 0 should return 42",
		);
		assert.strictEqual(
			getFunc(1)(),
			43,
			"Initial: Function at index 1 should return 43",
		);
		assert.strictEqual(
			getFunc(2)(),
			44,
			"Initial: Function at index 2 should return 44",
		);
		...
		assert.strictEqual(
			getFunc(3)(),
			42,
			"After copy: Function at index 3 should return 42",
		);
		...
		assert.strictEqual(
			getFunc(1)(),
			42,
			"After overlap copy: Function at index 1 should return 42",
		);
		assert.strictEqual(
			getFunc(2)(),
			43,
			"After overlap copy: Function at index 2 should return 43",
		);

		if (debug) console.log("✅ Table copy test passed");
		return true;
	} catch (error) {
