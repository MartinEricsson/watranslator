...
async function testMemorySizeGrow(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"memory-size-grow/memory-size-grow.wat",
			debug,
		);
		if (debug) {
		...
		if (typeof WebAssembly !== "undefined") {
			...
			try {
				...
				const { getMemorySize, growMemory, allocate, write, read, memory } =
					instance.exports;
				...
				console.log(
					`Initial memory size: ${initialSize} pages (${initialSize * 64}KB)`,
				);
				assert.strictEqual(
					initialSize,
					1,
					"Initial memory size should be 1 page",
				);
				...
				assert.strictEqual(
					addr,
					65536,
					"Allocation should start at page boundary",
				);
				...
				console.log(
					`Value written: 0x${testValue.toString(16)}, read: 0x${readValue.toString(16)}`,
				);
				...
				assert.strictEqual(
					memoryView[0],
					testValue,
					"Memory view should contain written value",
				);
				...
				assert.strictEqual(
					readValue,
					swappedValue,
					"Read should return the byte-swapped value",
				);
