...
const ABOVE_I64_MAX_AS_F64 = Number(2n ** 63n);
const BELOW_I64_MIN_AS_F64 = -Number(2n ** 63n) - 1;
const U64_OVERFLOW_BOUNDARY_AS_F64 = Number(2n ** 64n);

async function testTypeConversion(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"type-conversion/type-conversion.wat",
			debug,
		);
		...
		const tests = [
			...
			{
				name: "i32_trunc_sat_f32_s",
				expected: 1,
				func: "i32_trunc_sat_f32_s",
				params: [1.5],
			},
			{
				name: "i32_trunc_sat_f32_u",
				expected: 1,
			...
			},
			{
				...
				expected: 9223372036854775807n,
				func: "i64_trunc_sat_f64_s",
				params: [ABOVE_I64_MAX_AS_F64],
			}, // Max i64
			{
				name: "i64_trunc_sat_f64_s",
				expected: -9223372036854775808n,
				func: "i64_trunc_sat_f64_s",
				params: [BELOW_I64_MIN_AS_F64],
			}, // Min i64
			...
			}, // Negative becomes 0
			{
				...
				expected: 18446744073709551615n,
				func: "i64_trunc_sat_f64_u",
				params: [U64_OVERFLOW_BOUNDARY_AS_F64],
			}, // Max u64
		];
		...
		for (const { name, expected, func, params, unsigned } of tests) {
			...
			if (func.startsWith("f32_convert")) {
				...
				(func === "i64_trunc_sat_f64_u" || func === "i64_trunc_sat_f32_u") &&
				result === -1n
			) {
				// Generated by 🤖
