		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.early_return === "function") {
				...
				for (const test of testCases) {
					...
					console.log(
						`early_return(${test.input}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (allTestsPassed) {
					...
					console.error(
						"❌ Early return test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Early return test failed: early_return function not found in exports",
				);
				return false;
			}
