...
async function simdExtractLaneSTest(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"simd-extract-lane-s/simd-extract-lane-s.wat",
		);
		...
		console.assert(
			positiveResult === 127,
			`i8x16_extract_lane_s_positive returned ${positiveResult}, expected 127`,
		);
		...
		console.assert(
			negativeResult === -128,
			`i8x16_extract_lane_s_negative returned ${negativeResult}, expected -128`,
		);
		...
		console.assert(
			diffResult === -56,
			`i8x16_extract_lane_s_vs_u returned ${diffResult}, expected -56`,
		);
