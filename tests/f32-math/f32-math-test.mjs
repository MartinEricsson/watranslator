		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.abs === "function") {
				...
				for (const test of testCasesAbs) {
					...
					console.log(
						`abs(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.abs test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.neg === "function") {
				...
				for (const test of testCasesNeg) {
					...
					if (test.value === 0.0) {
						...
						console.log(
							`neg(${test.value}) = ${result} (negative zero?: ${isNegativeZero}), expected: -0.0 - ${isNegativeZero ? "✅" : "❌"}`,
						);
						if (!isNegativeZero) testsPassed = false;
					} else {
						const passed = isApproximatelyEqual(result, test.expected);
						console.log(
							`neg(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
						);
						if (!passed) testsPassed = false;
					}
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.neg test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.sqrt === "function") {
				...
				for (const test of testCasesSqrt) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`sqrt(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.sqrt test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.min === "function") {
				...
				for (const test of testCasesMin) {
					...
					if (test.expected === -0.0) {
						const isNegativeZero = Object.is(result, -0);
						console.log(
							`min(${test.a}, ${test.b}) = ${result} (negative zero?: ${isNegativeZero}), expected: -0.0 - ${isNegativeZero ? "✅" : "❌"}`,
						);
						if (!isNegativeZero) testsPassed = false;
					} else {
						const passed = isApproximatelyEqual(result, test.expected);
						console.log(
							`min(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
						);
						if (!passed) testsPassed = false;
					}
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.min test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.max === "function") {
				...
				for (const test of testCasesMax) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`max(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.max test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.ceil === "function") {
				...
				for (const test of testCasesCeil) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`ceil(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.ceil test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
			...
			if (typeof instance.exports.floor === "function") {
				...
				for (const test of testCasesFloor) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`floor(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.floor test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ f32.floor test failed: function not found in exports",
				);
				return false;
			}
			...
			if (typeof instance.exports.trunc === "function") {
				...
				for (const test of testCasesTrunc) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`trunc(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.trunc test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ f32.trunc test failed: function not found in exports",
				);
				return false;
			}
			...
			if (typeof instance.exports.nearest === "function") {
				...
				for (const test of testCasesNearest) {
					...
					const passed = isApproximatelyEqual(result, test.expected);
					console.log(
						`nearest(${test.value}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					...
					console.error(
						"❌ f32.nearest test failed: function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ f32.nearest test failed: function not found in exports",
				);
				return false;
			}
