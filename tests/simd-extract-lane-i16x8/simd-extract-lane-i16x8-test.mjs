...
async function simdExtractLaneI16x8Test(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"simd-extract-lane-i16x8/simd-extract-lane-i16x8.wat",
		);
		...
		console.assert(
			positiveResult === 32767,
			`i16x8_extract_lane_s_positive returned ${positiveResult}, expected 32767`,
		);

		// Test extract signed negative value
		const negativeResult = instance.exports.i16x8_extract_lane_s_negative();
		console.assert(
			negativeResult === -32768,
			`i16x8_extract_lane_s_negative returned ${negativeResult}, expected -32768`,
		);

		// Test extract unsigned value
		const unsignedResult = instance.exports.i16x8_extract_lane_u();
		console.assert(
			unsignedResult === 65535,
			`i16x8_extract_lane_u returned ${unsignedResult}, expected 65535`,
		);
		...
		console.assert(
			diffResult === -1,
			`i16x8_extract_lane_s_vs_u returned ${diffResult}, expected -1`,
		);
