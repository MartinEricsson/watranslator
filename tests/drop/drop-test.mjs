...
async function testDropInstruction(debug = false) {
	try {
		...
		console.assert(
			dropValueResultRes,
			`❌ drop_value should return 42, got ${dropValueResult}`,
		);
		...
		console.assert(
			dropComputationResultRes,
			`❌ drop_computation should return 10, got ${dropComputationResult}`,
		);
		...
		console.assert(
			dropParameterResultRes,
			`❌ drop_parameter should return 99, got ${dropParameterResult}`,
		);
		...
		console.assert(
			dropParameterNoReturnRes,
			`❌ drop_parameter_no_return should return undefined, got ${dropParameterNoReturn}`,
		);

		const dropParameterNoReturnParam =
			instance.exports.drop_parameter_no_return(42);
		const dropParameterNoReturnResParam =
			dropParameterNoReturnParam === undefined;
		console.assert(
			dropParameterNoReturnResParam,
			`❌ drop_parameter_no_return should return undefined, got ${dropParameterNoReturnParam}`,
		);
		...
		console.assert(
			dropDataRes,
			`❌ drop_data should return undefined, got ${dropData}`,
		);
