...
async function testAtomicMemory(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"atomic-memory/atomic-memory.wat",
			debug,
		);
		...
		for (const test of testCases) {
			const result = instance.exports.atomic_store_load(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_store_load(${test.address}, 0x${test.value.toString(16)}) = 0x${result.toString(16)} - ${passed ? "✅" : "❌"}`,
				);
			}
		...
		for (const test of i64TestCases) {
			const result = instance.exports.atomic_i64_store_load(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_i64_store_load(${test.address}, ${test.value}n) = ${result}n - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomic8UTestCases) {
			...
			if (debug || !passed) {
				console.log(
					`atomic_load8_u(${test.address}, 0x${test.value.toString(16)}) = 0x${result.toString(16)} - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomicStore8TestCases) {
			...
			const result = instance.exports.atomic_store8_load8_u(
				test.address,
				test.value,
			);
			const expectedValue = test.value & 0xff;
			...
			if (debug || !passed) {
				console.log(
					`atomic_store8_load8_u(${test.address}, 0x${test.value.toString(16)}) = 0x${result.toString(16)} - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomicStore16TestCases) {
			...
			const result = instance.exports.atomic_store16_load16_u(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_store16_load16_u(${test.address}, 0x${test.value.toString(16)}) = 0x${result.toString(16)} - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomicI64Store8TestCases) {
			...
			const result = instance.exports.atomic_i64_store8_load8_u(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_i64_store8_load8_u(${test.address}, ${test.value}n) = ${result}n - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomicI64Store16TestCases) {
			...
			const result = instance.exports.atomic_i64_store16_load16_u(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_i64_store16_load16_u(${test.address}, ${test.value}n) = ${result}n - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
		...
		for (const test of atomicI64Store32TestCases) {
			...
			const result = instance.exports.atomic_i64_store32_load32_u(
				test.address,
				test.value,
			);
			...
			if (debug || !passed) {
				console.log(
					`atomic_i64_store32_load32_u(${test.address}, ${test.value}n) = ${result}n - ${passed ? "✅" : "❌"}`,
				);
			}
			allPassed = allPassed && passed;
		}
