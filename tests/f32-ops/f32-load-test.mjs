...
async function testF32Load(debug = false) {
	...
	if (typeof instance.exports.load_float === "function") {
		...
		for (const test of testCases) {
			...
			console.assert(
				resultRes,
				`❌ load_float(${test.addr}) should return ${test.expected}, got ${result}`,
			);
			if (debug)
				console.log(
					`load_float(${test.addr}) = ${result}, expected: ${test.expected} - ${resultRes ? "✅" : "❌"}`,
				);
		}
	...
	console.error(
		"❌ f32.load test failed: load_float function not found in exports",
	);
