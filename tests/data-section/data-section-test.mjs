	...
	try {
		...
		for (let i = 0; i < text.length; i++) {
			...
			assert.strictEqual(
				actual,
				charCode,
				`Byte at offset ${16 + i} should be ${charCode} ('${text[i]}')`,
			);
			if (debug)
				console.log(
					`Byte at offset ${16 + i}: ${actual} ('${String.fromCharCode(actual)}') ✓`,
				);
		}
		...
		for (let i = 0; i < specialChars.length; i++) {
			...
			assert.strictEqual(
				actual,
				expected,
				`Special character at offset ${64 + i} should be ${expected}`,
			);
			if (debug)
				console.log(`Special character at offset ${64 + i}: ${actual} ✓`);
		}
		...
		for (let i = 0; i < hexValues.length; i++) {
			...
			assert.strictEqual(
				actual,
				expected,
				`Hex value at offset ${128 + i} should be ${expected}`,
			);
			if (debug)
				console.log(`Hex value at offset ${128 + i}: ${actual.toString(16)} ✓`);
		}
		...
		for (let i = 0; i < mixString.length; i++) {
			...
			assert.strictEqual(
				actual,
				expected,
				`Character at offset ${256 + i} should be ${expected}`,
			);
			if (debug)
				console.log(
					`Character at offset ${256 + i}: ${actual} ('${String.fromCharCode(actual)}') ✓`,
				);
		}
		...
		for (const check of byteOffsets) {
			...
			assert.strictEqual(
				actual,
				check.expected,
				`Byte at offset ${check.offset} should be ${check.expected}`,
			);
			if (debug)
				console.log(
					`Byte at offset ${check.offset}: ${actual} ${actual >= 32 && actual <= 126 ? `('${String.fromCharCode(actual)}')` : ""} ✓`,
				);
		}
