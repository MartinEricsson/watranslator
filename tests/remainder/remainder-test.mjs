		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.remainder_signed === "function") {
				...
				for (const test of testCasesSigned) {
					...
					console.log(
						`remainder_signed(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (testsPassed) {
					console.log(
						"✅ Signed remainder test: remainder_signed function works correctly!",
					);
				} else {
					console.error(
						"❌ Signed remainder test failed: remainder_signed function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Signed remainder test failed: remainder_signed function not found in exports",
				);
				return false;
			}
			...
			if (typeof instance.exports.remainder_unsigned === "function") {
				...
				for (const test of testCasesUnsigned) {
					...
					const passed = result === test.expected;
					console.log(
						`remainder_unsigned(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
					if (!passed) testsPassed = false;
				}
				...
				if (testsPassed) {
					console.log(
						"✅ Unsigned remainder test: remainder_unsigned function works correctly!",
					);
				} else {
					console.error(
						"❌ Unsigned remainder test failed: remainder_unsigned function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Unsigned remainder test failed: remainder_unsigned function not found in exports",
				);
				return false;
			}
