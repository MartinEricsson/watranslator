...
async function testComprehensiveSourceMap() {
	...
	try {
		...
		const hasBothFunctions = func0Mappings.length > 0 && func1Mappings.length > 0;
		...
		const func0Valid = func0Mappings.every((entry, i, arr) => 
			i === 0 || entry.bodyOffset >= arr[i-1].bodyOffset);
		const func1Valid = func1Mappings.every((entry, i, arr) => 
			i === 0 || entry.bodyOffset >= arr[i-1].bodyOffset);
