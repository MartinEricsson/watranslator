...
async function testAlignmentValidation(debug = false) {
	...
	for (const test of ALIGNMENT_TESTS) {
		...
		try {
			...
			if (test.shouldPass) {
				console.log(
					`✅ ${test.name}: PASSED (compilation succeeded as expected)`,
				);
				passed++;
			} else {
				console.error(
					`❌ ${test.name}: FAILED (expected error but got successful compilation)`,
				);
				failed++;
			}
		} catch (error) {
			if (!test.shouldPass && error.message.includes(test.expectedError)) {
				console.log(
					`✅ ${test.name}: PASSED (got expected error: "${test.expectedError}")`,
				);
				passed++;
			} else if (!test.shouldPass) {
				console.error(
					`❌ ${test.name}: FAILED (error message doesn't match expected)`,
				);
				...
				console.error(
					`❌ ${test.name}: FAILED (expected success but got error)`,
				);
