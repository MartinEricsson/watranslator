...
async function testSIMDi8x16Ops(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-i8x16-ops/simd-i8x16-ops.wat",
			debug,
		);
		...
		if (popcntResult !== 1) {
			throw new Error(
				`i8x16.popcnt test failed: expected 1, got ${popcntResult}`,
			);
		}
		...
		if (allTrueResult !== 1) {
			throw new Error(
				`i8x16.all_true test failed: expected 1, got ${allTrueResult}`,
			);
		}
		...
		if (bitmaskResult !== 5) {
			throw new Error(
				`i8x16.bitmask test failed: expected 5, got ${bitmaskResult}`,
			);
		}
		...
		if (narrowSResult !== -128) {
			throw new Error(
				`i8x16.narrow_i16x8_s test failed: expected -128, got ${narrowSResult}`,
			);
		}
		...
		if (narrowUResult !== 200) {
			throw new Error(
				`i8x16.narrow_i16x8_u test failed: expected 200, got ${narrowUResult}`,
			);
		}
		...
		if (shrSResult !== -64) {
			throw new Error(
				`i8x16.shr_s test failed: expected -64, got ${shrSResult}`,
			);
		}
		...
		if (shrUResult !== 64) {
			throw new Error(
				`i8x16.shr_u test failed: expected 64, got ${shrUResult}`,
			);
		}
		...
		if (addSatSResult !== 127) {
			throw new Error(
				`i8x16.add_sat_s test failed: expected 127, got ${addSatSResult}`,
			);
		}
		...
		if (addSatUResult !== -1) {
			throw new Error(
				`i8x16.add_sat_u test failed: expected -1, got ${addSatUResult}`,
			);
		}
		...
		if (subSatSResult !== -128) {
			throw new Error(
				`i8x16.sub_sat_s test failed: expected -128, got ${subSatSResult}`,
			);
		}
		...
		if (subSatUResult !== 0) {
			throw new Error(
				`i8x16.sub_sat_u test failed: expected 0, got ${subSatUResult}`,
			);
		}
		...
		if (minSResult !== -10) {
			throw new Error(
				`i8x16.min_s test failed: expected -10, got ${minSResult}`,
			);
		}
		...
		if (minUResult !== 10) {
			throw new Error(
				`i8x16.min_u test failed: expected 10, got ${minUResult}`,
			);
		}
		...
		if (maxSResult !== 10) {
			throw new Error(
				`i8x16.max_s test failed: expected 10, got ${maxSResult}`,
			);
		}
		...
		if (maxUResult !== -10) {
			throw new Error(
				`i8x16.max_u test failed: expected -10, got ${maxUResult}`,
			);
		}
		...
		if (avgrUResult !== 15) {
			throw new Error(
				`i8x16.avgr_u test failed: expected 15, got ${avgrUResult}`,
			);
		}
