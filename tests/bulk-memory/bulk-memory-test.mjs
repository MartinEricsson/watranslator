...
async function testBulkMemory(debug = false) {
	try {
		...
		assert(
			ast[0].datas && ast[0].datas.length > 0,
			"Data sections should be present in the AST",
		);
		...
		for (let i = 8; i < wasmBuffer.length - 2; i++) {
			if (wasmBuffer[i] === DATA_COUNT_SECTION_ID) {
				...
				if (wasmBuffer[i + 2] === expectedDataCount) {
					...
					if (debug)
						console.log(
							`Found data count section at offset ${i} with count ${wasmBuffer[i + 2]}`,
						);
					break;
				}
			}
		...
		assert(
			foundDataCountSection,
			`Data count section with count=${expectedDataCount} should exist in the binary`,
		);

		// Get exported functions and memory
		const { readByte, readInt, readI64, readI64SignedByte, memory } =
			instance.exports;
		...
		assert.strictEqual(
			intZero,
			50462976,
			"Int at offset 0 should be 50462976 (0x03020100)",
		);
		...
		assert.strictEqual(
			intOne,
			67305985,
			"Int at offset 1 should be 67305985 (0x04030201)",
		);
		...
		assert.strictEqual(
			i64Zero,
			506097522914230528n,
			"i64 at offset 0 should be 506097522914230528n (0x0706050403020100)",
		);
		...
		assert.strictEqual(
			i64One,
			578437695752307201n,
			"i64 at offset 1 should be 578437695752307201n (0x0807060504030201)",
		);
		...
		for (let i = 0; i < text.length; i++) {
			...
			assert.strictEqual(
				actual,
				charCode,
				`Byte at offset ${100 + i} should be ${charCode} ('${text[i]}')`,
			);
			if (debug)
				console.log(
					`Byte at offset ${100 + i}: ${actual} ('${String.fromCharCode(actual)}') ✓`,
				);
		}
		...
		for (let i = 0; i < testData.length; i++) {
			...
			assert.strictEqual(
				actual,
				expected,
				`Byte at offset ${200 + i} should be ${expected}`,
			);
			if (debug)
				console.log(`Byte at offset ${200 + i}: ${actual.toString(16)} ✓`);
		}
