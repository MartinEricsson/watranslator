...
function processDataBytes(ast) {
	...
	if (module?.datas && module.datas.length > 0) {
		...
		if (dataSection?.bytes) {
			...
			console.log(
				"Fixed data section bytes:",
				dataSection.bytes.processedBytes,
			);
		}
	}
	...
	try {
		...
		if (typeof WebAssembly !== "undefined") {
			...
			try {
				...
				console.log(
					`load_i32(0) = 0x${result1.toString(16)}, expected: 0x${expected1.toString(16)} - ${result1 === expected1 ? "✅" : "❌"}`,
				);
				...
				const expected2 = -1;
				console.log(
					`load_i32_8_s(8) = ${result2}, expected: ${expected2} - ${result2 === expected2 ? "✅" : "❌"}`,
				);
				...
				const expected3 = 255;
				console.log(
					`load_i32_8_u(8) = ${result3}, expected: ${expected3} - ${result3 === expected3 ? "✅" : "❌"}`,
				);
				...
				const expected4 = -257;
				console.log(
					`load_i32_16_s(8) = ${result4}, expected: ${expected4} - ${result4 === expected4 ? "✅" : "❌"}`,
				);
				...
				console.log(
					`load_i32_16_u(8) = ${result5}, expected: ${expected5} - ${result5 === expected5 ? "✅" : "❌"}`,
				);
				...
				console.log(
					`load_i32_offset(4) = 0x${result6.toString(16)}, expected: 0x${expected6.toString(16)} - ${result6 === expected6 ? "✅" : "❌"}`,
				);
