...
async function testSIMDMemory(debug = false) {
	const log = logger(debug);
	try {
		const { wasmBuffer } = await readTestData(
			"simd-memory/simd-memory.wat",
			debug,
		);
		...
		log(
			`test_v128_load_offset: got ${loadOffsetResult.toString(16)}, expected 0xaa`,
		);
		if (loadOffsetResult !== 0xaa) {
			throw new Error(
				`v128.load with offset test failed: expected 0xaa, got ${loadOffsetResult.toString(16)}`,
			);
		}
		...
		if (storeResult !== 0x42) {
			throw new Error(
				`v128.store test failed: expected 0x42, got ${storeResult}`,
			);
		}
		...
		if (load8SplatResult !== 3) {
			throw new Error(
				`v128.load8_splat test failed: expected 3, got ${load8SplatResult}`,
			);
		}
		...
		if (load16SplatResult !== 0x0201) {
			throw new Error(
				`v128.load16_splat test failed: expected 0x0201, got ${load16SplatResult.toString(16)}`,
			);
		}
		...
		log(
			`test_v128_load32_splat: got ${load32SplatResult.toString(16)}, expected 0x03020100`,
		);
		if (load32SplatResult !== 0x03020100) {
			throw new Error(
				`v128.load32_splat test failed: expected 0x03020100, got ${load32SplatResult.toString(16)}`,
			);
		}
		...
		log(
			`test_v128_load64_splat: got ${load64SplatResult.toString(16)}, expected ${expected64.toString(16)}`,
		);
		if (load64SplatResult !== expected64) {
			throw new Error(
				`v128.load64_splat test failed: expected ${expected64.toString(16)}, got ${load64SplatResult.toString(16)}`,
			);
		}
		...
		if (load8LaneResult !== 5) {
			throw new Error(
				`v128.load8_lane test failed: expected 5, got ${load8LaneResult}`,
			);
		}
		...
		if (store8LaneResult !== 0x42) {
			throw new Error(
				`v128.store8_lane test failed: expected 0x42, got ${store8LaneResult}`,
			);
		}
		...
		log(
			`test_v128_load32_zero: got ${load32ZeroResult.toString(16)}, expected 0x03020100`,
		);
		if (load32ZeroResult !== 0x03020100) {
			throw new Error(
				`v128.load32_zero test failed: expected 0x03020100, got ${load32ZeroResult.toString(16)}`,
			);
		}

		// Test v128.load8x8_s
		const load8x8SResult = instance.exports.test_v128_load8x8_s();
		log(`test_v128_load8x8_s: got ${load8x8SResult}, expected 7`);
		if (load8x8SResult !== 7) {
			throw new Error(
				`v128.load8x8_s test failed: expected 7, got ${load8x8SResult}`,
			);
		}
		// Generated by 🤖
