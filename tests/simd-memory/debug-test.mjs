...
async function debugSIMDOffset(debug = true) {
	...
	try {
		...
		log(
			`Byte at offset 0: 0x${byte0.toString(16).padStart(2, "0")}, expected 0x00`,
		);

		const byte16 = instance.exports.read_byte_at_offset(16);
		log(
			`Byte at offset 16: 0x${byte16.toString(16).padStart(2, "0")}, expected 0xaa`,
		);

		// Now test the SIMD load with offset
		const loadOffsetResult = instance.exports.test_v128_load_offset();
		log(
			`test_v128_load_offset: got 0x${loadOffsetResult.toString(16).padStart(2, "0")}, expected 0xaa`,
		);

		if (loadOffsetResult !== 0xaa) {
			throw new Error(
				`v128.load with offset test failed: expected 0xaa, got 0x${loadOffsetResult.toString(16).padStart(2, "0")}`,
			);
		}
