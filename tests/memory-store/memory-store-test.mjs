	...
	try {
		...
		if (typeof WebAssembly !== "undefined") {
			...
			try {
				...
				console.log(
					`store(${address1}, 0x${testValue1.toString(16)}) -> load(${address1}) = 0x${result1.toString(16)} - ${result1 === testValue1 ? "✅" : "❌"}`,
				);
				...
				console.log(
					`store_with_align_offset(${address2}, 0x${testValue2.toString(16)}) -> load(8) = 0x${result2.toString(16)}`,
				);
				...
				console.log(
					`Memory bytes at offset 8: [${memoryBytes.map((b) => `0x${b.toString(16)}`).join(", ")}]`,
				);
				const expectedBytes = [0xdd, 0xcc, 0xbb, 0xaa]; // Little-endian representation of 0xAABBCCDD
				const correctByteOrder = expectedBytes.every(
					(byte, i) => byte === memoryBytes[i],
				);
				console.log(
					`Bytes in correct order: ${correctByteOrder ? "✅" : "❌"}`,
				);
				...
				console.log(
					`store8(${address3}, 0x${testValue3.toString(16)}) -> (load(${address3}) & 0xFF) = 0x${result3.toString(16)} - ${result3 === testValue3 ? "✅" : "❌"}`,
				);
				...
				console.log(
					`store16(${address4}, 0x${testValue4.toString(16)}) -> (load(${address4}) & 0xFFFF) = 0x${result4.toString(16)} - ${result4 === testValue4 ? "✅" : "❌"}`,
				);
