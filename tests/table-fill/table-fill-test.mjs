...
async function testTableFill(debug = false) {
	try {
		const { wasmBuffer, ast } = await readTestData(
			"table-fill/table-fill.wat",
			debug,
		);

		// Verify AST structure
		assert(
			ast[0].tables && ast[0].tables.length > 0,
			"Table should be present in AST",
		);
		...
		assert.strictEqual(
			...
			42,
			"After fill: Function at index 1 should return 42",
		);
		assert.strictEqual(
			instance.exports.get_func(2),
			42,
			"After fill: Function at index 2 should return 42",
		);
		assert.strictEqual(
			instance.exports.get_func(3),
			42,
			"After fill: Function at index 3 should return 42",
		);
		...
		assert.strictEqual(
			instance.exports.get_func(1),
			42,
			"After second fill: Function at index 1 should still return 42",
		);
		assert.strictEqual(
			instance.exports.get_func(2),
			43,
			"After second fill: Function at index 2 should return 43",
		);
		assert.strictEqual(
			instance.exports.get_func(3),
			42,
			"After second fill: Function at index 3 should still return 42",
		);
		...
		try {
			...
			assert(
				...
				"Expected WebAssembly.RuntimeError when calling null function reference",
			);
		}
		...
		assert.strictEqual(
			instance.exports.get_func(2),
			43,
			"After null fill: Function at index 2 should still return 43",
		);
