...
async function simdReplaceAllLaneTest(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-replace-lane/simd-replace-lane-all.wat",
		);
		...
		const tests = [
			{
				...
				expected: 12345,
				functionName: "i16x8_replace_lane_test",
			},
			{
				name: "i32x4.replace_lane",
				expected: 42424242,
				functionName: "i32x4_replace_lane_test",
			},
			{
				name: "i64x2.replace_lane",
				expected: 9876543210n,
				functionName: "i64x2_replace_lane_test",
			},
			{
				name: "f32x4.replace_lane",
				expected: Math.PI,
				functionName: "f32x4_replace_lane_test",
			},
			{
				name: "f64x2.replace_lane",
				expected: Math.E,
				functionName: "f64x2_replace_lane_test",
			},
		];
