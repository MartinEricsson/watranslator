...
async function simdReplaceLaneTest(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-replace-lane/simd-replace-lane.wat",
		);
		...
		const tests = [
			{
				...
				expected: 42,
				functionName: "i8x16_replace_lane_test",
			},
			{
				name: "i8x16.replace_lane_multiple",
				expected: -56,
				functionName: "i8x16_replace_lane_multiple",
			},
			{
				name: "i8x16.replace_lane_negative",
				expected: -30,
				functionName: "i8x16_replace_lane_negative",
			},
		];
