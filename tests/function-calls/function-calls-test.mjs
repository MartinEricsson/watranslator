...
async function testFunctionCalls(debug = true) {
	try {
		const { wasmBuffer } = await readTestData(
			"function-calls/function-calls.wat",
			debug,
		);
		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.multiplyAndAdd === "function") {
				...
				if (result === 38) {
					...
					console.error(
						`❌ Function calls test failed: Expected 38, got ${result}`,
					);
					return false;
				}
			} else {
				console.error(
					"❌ Function calls test failed: multiplyAndAdd function not found in exports",
				);
				return false;
			}
		}
