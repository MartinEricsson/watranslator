		...
		if (typeof WebAssembly !== "undefined") {
			...
			if (typeof instance.exports.subtract === "function") {
				...
				for (const test of testCases) {
					...
					console.log(
						`subtract(${test.a}, ${test.b}) = ${result}, expected: ${test.expected} - ${passed ? "✅" : "❌"}`,
					);
				...
				if (testsPassed) {
					console.log(
						"✅ Subtraction test: subtract function works correctly!",
					);
				} else {
					console.error(
						"❌ Subtraction test failed: subtract function returned incorrect results",
					);
					return false;
				}
			} else {
				console.error(
					"❌ Subtraction test failed: subtract function not found in exports",
				);
				return false;
			}
