...
async function testSIMDSwizzleSplat(debug = false) {
	...
	try {
		const { wasmBuffer } = await readTestData(
			"simd-swizzle-splat/simd-swizzle-splat.wat",
			debug,
		);
		...
		if (swizzleResult !== 1) {
			throw new Error(
				`i8x16.swizzle test failed: expected 1, got ${swizzleResult}`,
			);
		}
		...
		if (i8SplatResult !== 168) {
			throw new Error(
				`i8x16.splat test failed: expected 168, got ${i8SplatResult}`,
			);
		}
		...
		if (i16SplatResult !== 1000) {
			throw new Error(
				`i16x8.splat test failed: expected 1000, got ${i16SplatResult}`,
			);
		}
		...
		if (i32SplatResult !== 12345) {
			throw new Error(
				`i32x4.splat test failed: expected 12345, got ${i32SplatResult}`,
			);
		}
		...
		if (i64SplatResult !== BigInt(9876543210)) {
			throw new Error(
				`i64x2.splat test failed: expected 9876543210n, got ${i64SplatResult}`,
			);
		}
		...
		if (Math.abs(f32SplatResult - Math.PI) > 0.0001) {
			throw new Error(
				`f32x4.splat test failed: expected ≈3.14159, got ${f32SplatResult}`,
			);
		}
		...
		if (Math.abs(f64SplatResult - Math.E) > 0.0001) {
			throw new Error(
				`f64x2.splat test failed: expected ≈2.71828, got ${f64SplatResult}`,
			);
		}
