...
async function testSIMDConst(debug = false) {
	...
	try {
		...
		const {
			...
			v128_const_f32x4,
			v128_const_f64x2,
			v128_const_i64x2_signed_bit,
			v128_const_i64x2_max,
			v128_const_i64x2_min,
			v128_const_i64x2_alternating,
		...
		log(
			`v128.const test with mixed values: ${result2 === 4 ? "PASSED" : "FAILED"}`,
		);
		if (result2 !== 4) {
		...
		log(
			`v128.const i64x2 signed bit test: ${resultSigned === minInt64 ? "PASSED" : "FAILED"}`,
		);
		if (resultSigned !== minInt64) {
			throw new Error(
				`v128.const i64x2 signed bit failed: expected ${minInt64}, got ${resultSigned}`,
			);
		} // Generated by 🤖
		...
		log(
			`v128.const i64x2 max value test: ${resultMax === maxInt64 ? "PASSED" : "FAILED"}`,
		);
		if (resultMax !== maxInt64) {
			throw new Error(
				`v128.const i64x2 max value failed: expected ${maxInt64}, got ${resultMax}`,
			);
		}
		...
		log(
			`v128.const i64x2 min value test: ${resultMin === minInt64 ? "PASSED" : "FAILED"}`,
		);
		if (resultMin !== minInt64) {
			throw new Error(
				`v128.const i64x2 min value failed: expected ${minInt64}, got ${resultMin}`,
			);
		}
		...
		log(
			`v128.const i64x2 alternating pattern test: ${resultAlt === alternatingPattern ? "PASSED" : "FAILED"}`,
		);
		if (resultAlt !== alternatingPattern) {
			throw new Error(
				`v128.const i64x2 alternating pattern failed: expected ${alternatingPattern}, got ${resultAlt}`,
			);
		} // Generated by 🤖
		...
		log(
			`v128.const i64x2 32-bit boundary test: ${resultBoundary === boundary32Bit ? "PASSED" : "FAILED"}`,
		);
		if (resultBoundary !== boundary32Bit) {
			throw new Error(
				`v128.const i64x2 32-bit boundary failed: expected ${boundary32Bit}, got ${resultBoundary}`,
			);
		} // Generated by 🤖
