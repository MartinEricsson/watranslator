	...
	try {
		try {
			const { wasmBuffer } = await readTestData(
				"simd-const/simd-const-error.wat",
				debug,
			);
			...
			console.error(
				"FAILED: Compilation succeeded but should have failed due to invalid v128.const immediates",
			);
			return false;
		} catch (error) {
			...
			if (error.message.includes("v128.const requires exactly 16 i8 values")) {
				console.log(
					"PASSED: Correctly caught error with invalid number of immediates for v128.const",
				);
				return true;
			}
			...
			// Wrong error type
			console.error(
				"FAILED: Got error but not for the expected reason:",
				error.message,
			);
			return false;
		}
