...
async function testMultiMemoryIndexes(debug = false) {
	try {
		...
		const { wasmBuffer: wasmBuffer0 } = await readTestData(
			"multi-memory/memory0.wat",
			debug,
		);
		const { instance: instance0 } = await WebAssembly.instantiate(
			wasmBuffer0,
			imports,
		);

		// Load and instantiate the second module (memory1)
		const { wasmBuffer: wasmBuffer1 } = await readTestData(
			"multi-memory/memory1.wat",
			debug,
		);
		const { instance: instance1 } = await WebAssembly.instantiate(
			wasmBuffer1,
			imports,
		);
		...
		const tests = [
			{
				...
				execute: () => {
					...
					if (debug) {
						console.log(
							`Memory0 storage: Expected: ${value}, Got: func=${result}, direct=${directResult}`,
						);
					}

					if (result !== value || directResult !== value) {
						console.error(
							`❌ Memory0 storage failed. Expected: ${value}, Got: func=${result}, direct=${directResult}`,
						);
						return false;
					}
					return true;
			...
			},
			{
				...
				execute: () => {
					...
					if (debug) {
						console.log(
							`Memory1 storage: Expected: ${value}, Got: func=${result}, direct=${directResult}`,
						);
					}

					if (result !== value || directResult !== value) {
						console.error(
							`❌ Memory1 storage failed. Expected: ${value}, Got: func=${result}, direct=${directResult}`,
						);
						return false;
					}
					return true;
			...
			},
			{
				...
				execute: () => {
					...
					if (debug) {
						console.log(
							`Memory isolation: memory0[${addr}]=${direct0}, memory1[${addr}]=${direct1}`,
						);
					}

					if (result0 !== value0 || direct0 !== value0) {
						console.error(
							`❌ Memory0 isolation failed. Expected: ${value0}, Got: func=${result0}, direct=${direct0}`,
						);
						return false;
					}
					...
					if (result1 !== value1 || direct1 !== value1) {
						console.error(
							`❌ Memory1 isolation failed. Expected: ${value1}, Got: func=${result1}, direct=${direct1}`,
						);
						return false;
					}
