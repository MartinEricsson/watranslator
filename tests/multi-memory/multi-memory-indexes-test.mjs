// filepath: /Users/martinericsson/dev/watranslator/tests/multi-memory/multi-memory-indexes-test.mjs
import { readTestData } from "../test-utils.mjs";

async function testMultiMemoryIndexes(debug = false) {
    try {
        ...
        const tests = [
            {
                name: "store and load from memory0 with index",
                execute: () => {
                    const addr = 0;
                    const value = 42;
            ...
            {
                name: "store and load from memory1 with index",
                execute: () => {
                    const addr = 4;
                    const value = 99;
        ...
        if (allTestsPassed) {
            console.log("✅ All multi-memory indexes tests passed!");
            return true;
        } else {
            console.error("❌ Some multi-memory indexes tests failed.");
            return false;
        }
    } catch (error) {
        console.error('❌ Error testing multi-memory indexes:', error);
        return false;
    }
}

export default testMultiMemoryIndexes; // Generated by 🤖