...
async function testTypeConversionF64(debug = false) {
	try {
		const { wasmBuffer } = await readTestData(
			"type-conversion-f64/type-conversion-f64.wat",
			debug,
		);
		...
		for (const { name, expected, func, params } of tests) {
			...
			if (debug || !resultRes) {
				console.log(
					`${name} with params [${params}]: result=${result}, expected=${expected}, ${resultRes ? "✅" : "❌"}`,
				);
			}
		}
