		...
		for (const global of nonImportedGlobals) {
			...
			if (global.init) {
				if (global.init.type === "i32.const") {
					...
					globalEntries.push(...encodeSLEB128(global.init.value));
				} else if (global.init.type === "f32.const") {
					globalEntries.push(INSTR.F32_CONST);
					globalEntries.push(
						...(global.init.bits !== undefined
							? encodeF32Bits(global.init.bits)
							: encodeF32(global.init.value)),
					);
					...
					globalEntries.push(
						...(global.init.bits !== undefined
							? encodeF64Bits(global.init.bits)
							: encodeF64(global.init.value)),
					);
				...
				if (global.type === "i32") {
					...
					globalEntries.push(INSTR.F32_CONST);
					globalEntries.push(...encodeF32(0.0));
