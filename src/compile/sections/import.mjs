...
export function importSection(module, binary) {
    ...
    // Check for imported globals declared inline
    ...
    // NEW: Include memory imports captured in module.imports (previously ignored)
    if (Array.isArray(module.imports) && module.imports.length > 0) {
        for (const imp of module.imports) {
            if (imp && imp.kind === 'memory') {
                // Avoid duplicating if already present
                const already = imports.some(i => i.kind === 'memory' && i.module === imp.module && i.field === imp.field);
                if (!already) {
                    imports.push({
                        kind: 'memory',
                        module: imp.module,
            ...
            } else if (imp && imp.kind === 'global' && !imports.some(i => i.kind === 'global' && i.module === imp.module && i.field === imp.field)) {
                // Support for global imports that may only exist in module.imports
                if (imp.globalType) {
                    imports.push({
                        kind: 'global',
                        module: imp.module,
    ...
    const importSectionBytes = [SECTION.IMPORT];
