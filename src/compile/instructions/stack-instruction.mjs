...
export function compileStackInstruction(instr, binary) { // Changed body to binary
    const stackInstrEntry = STACK_INSTR.find(i => i.type === instr.type);
    
    if (stackInstrEntry) {
        if (instr.loc && instr.loc.line != null) {
            addMapping(instr.loc.line, instr.loc.column, binary.length);
        }
        binary.push(stackInstrEntry.instr);
        return true;
    }
