	...
	if (instr.op === "get_local" || instr.op === "local.get") {
		...
		try {
			...
			throw createError(
				...
				func,
				module,
				`Cannot find local variable: ${instr.operand}. Make sure the variable is defined with a param or local declaration.`,
			);
		}
	} else if (instr.op === "set_local" || instr.op === "local.set") {
		...
		try {
			...
			throw createError(
				...
				func,
				module,
				`Cannot find local variable: ${instr.operand}`,
			);
		}
	} else if (instr.op === "local.tee") {
		// Generated by 🤖
		...
		if (globalIndex === -1) {
			throw createError(
				...
				func,
				module,
				`Unknown global variable: ${instr.operand}. Global variables must be defined before use.`,
			);
		}
		...
		if (instr.operand.startsWith("$")) {
			...
			if (globalIndex !== -1 && !moduleGlobals[globalIndex].mutable) {
				throw createError(
					...
					func,
					module,
					`Cannot set immutable global: ${instr.operand}. Add (mut) qualifier to make the global mutable.`,
				);
			}
		} else {
			globalIndex = Number.parseInt(instr.operand, 10) || 0;
