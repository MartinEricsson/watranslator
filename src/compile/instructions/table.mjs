...
export function compileTableInstruction(instr, func, module, body) {
		...
		try {
			...
			if (module.elements && instr.elementIndex >= module.elements.length) {
				throw createError(
					...
					func,
					module,
					`Element segment ${instr.elementIndex} does not exist. Module has ${module.elements.length} element segments.`,
				);
			}
			...
			if (module.tables && instr.tableIndex >= module.tables.length) {
				throw createError(
					...
					func,
					module,
					`Table index ${instr.tableIndex} is out of bounds. Module has ${module.tables.length} tables.`,
				);
			}
	...
	} else if (instr.op === "table.copy") {
		// Generated by 🤖
		try {
			// Validate table indices exist
			if (
				module.tables &&
				(instr.destTableIndex >= module.tables.length ||
					instr.srcTableIndex >= module.tables.length)
			) {
				throw createError(
					instr,
					func,
		...
		try {
			...
			if (module.elements && instr.elementIdx >= module.elements.length) {
				throw createError(
					...
					func,
					module,
					`Element segment ${instr.elementIdx} does not exist. Module has ${module.elements.length} element segments.`,
				);
			}
