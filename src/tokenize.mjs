...
export function tokenize(fullInput) {
    ...
    while (i < input.length) {
        ...
        if (inString) {
            ...
            if (char === '"' && input[i - 1] !== '\\') {
                ...
                tokens.push({ value: currentToken, line: stringStartLine + 1, column: stringStartCol + 1 });
                currentToken = "";
                inString = false;
            }
        } else {
            ...
            if (char === '"') {
                ...
                if (currentToken) {
                    ...
                    for (const t of splitTokens) {
                        tokens.push({ value: t, line: tokenStartLine + 1, column: tokenStartCol + 1 });
                    }
                    currentToken = "";
                }
                currentToken = char;
                ...
                tokens.push({ value: char, line: currentLine + 1, column: currentCol + 1 });
                ...
                if (currentToken) {
                    ...
                    if (memInstructionRegex.test(currentToken)) {
                        ...
                        if (attrToken.trim() && validAttribute) {
                            ...
                            tokens.push({ value: currentToken + attrToken, line: tokenStartLine + 1, column: tokenStartCol + 1 });
    ...
    if (currentToken) {
        if (inString) {
            tokens.push({ value: currentToken, line: stringStartLine + 1, column: stringStartCol + 1 }); // Push incomplete string as-is
        } else {
            const splitTokens = currentToken.trim().split(/\s+/).filter(t => t);
            for (const t of splitTokens) {
                tokens.push({ value: t, line: tokenStartLine + 1, column: tokenStartCol + 1 });
    ...
    const filteredTokens = tokens.filter(token => token.value.length > 0);
