		...
		if (inString) {
			...
			if (char === '"') {
				...
				if (currentToken) {
					...
					const splitTokens = currentToken
						.trim()
						.split(/\s+/)
						.filter((t) => t);
				...
				if (currentToken) {
					...
					if (isMemargInstructionToken(currentToken)) {
						...
						while (j < input.length && input[j] !== ")" && input[j] !== "\n") {
							attrToken += input[j];
							if (
								attrToken.includes("offset=") ||
								attrToken.includes("align=")
							) {
								validAttribute = true;
							}
							j++;
