...
export function parseModule(tape) {
	const { atEnd, getCurrentCursor, peekToken, skipToken } = tape;
		...
		if (peekToken() === "(") {
			...
			if (!atEnd()) {
				...
				if (sectionType === "func") {
					...
					const importDecl = parseImport(tape);
					if (importDecl) {
					...
					const exportItem = parseExport(tape);
					if (exportItem?.name) {
