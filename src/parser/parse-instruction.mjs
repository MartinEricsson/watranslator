...
export function parseInstruction(blockLabels) {
    ...
    else if (instrType === 'local.get' || instrType === 'get_local') {
        const operand = getToken();
        return { type: 'get_local', operand, position };
    } else if (instrType === 'local.set' || instrType === 'set_local') {
        const operand = getToken();
        return { type: 'set_local', operand, position };
    } else if (instrType === 'local.tee') {
        const operand = getToken();
        return { type: 'local.tee', operand, position };
    } else if (instrType === 'global.get' || instrType === 'get_global') {
        const operand = getToken();
        return { type: 'global.get', operand, position };
    } else if (instrType === 'global.set' || instrType === 'set_global') {
        const operand = getToken();
        return { type: 'global.set', operand, position };
    } else if (instrType === 'i32.const') {
    ...
    // Handle elem.drop instruction
    ...
    } else if (instrType === 'call_indirect') {
        // Generated by 🤖
        let params = [];
        let results = [];
        let typeRef = null;
        ...
        if (!atEnd() && /^\d+$/.test(peekToken())) {
            tableIndex = parseInt(getToken(), 10);
        }
