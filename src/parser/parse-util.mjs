...
export function createError(message) {
    const pos = getCurrentCursor(); // pos can be null if atEnd() was true before tape was initialized
    const currentToken = peekToken();
    const tokenValue = currentToken ? `"${currentToken.value}"` : "end of input";
    
    let line = pos ? pos.line : (currentToken ? currentToken.line : 'N/A');
    let column = pos ? pos.column : (currentToken ? currentToken.column : 'N/A');

    // If pos is null because tape was empty/uninitialized, try to get from token if possible
    if (pos === null && currentToken) {
        line = currentToken.line;
        column = currentToken.column;
    }


    return new Error(`PARSE ERROR: ${message} at line ${line}, column ${column} near ${tokenValue}`);
}
