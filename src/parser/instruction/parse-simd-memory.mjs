...
export function parseSIMDMemory(instrToken, position) {
	...
	if (
		instrToken === "v128.load" ||
		instrToken === "v128.store" ||
		instrToken === "v128.load8x8_s" ||
		instrToken === "v128.load8x8_u" ||
		instrToken === "v128.load16x4_s" ||
		instrToken === "v128.load16x4_u" ||
		instrToken === "v128.load32x2_s" ||
		instrToken === "v128.load32x2_u" ||
		instrToken === "v128.load8_splat" ||
		instrToken === "v128.load16_splat" ||
		instrToken === "v128.load32_splat" ||
		instrToken === "v128.load64_splat" ||
		instrToken === "v128.load32_zero" ||
		instrToken === "v128.load64_zero"
	) {
	...
	if (
		instrToken === "v128.load8_lane" ||
		instrToken === "v128.load16_lane" ||
		instrToken === "v128.load32_lane" ||
		instrToken === "v128.load64_lane" ||
		instrToken === "v128.store8_lane" ||
		instrToken === "v128.store16_lane" ||
		instrToken === "v128.store32_lane" ||
		instrToken === "v128.store64_lane"
	) {
		let laneIndex = 0;

		const { align, offset } = parseOffsetAndAlign(); // Generated by 🤖
		...
		if (!atEnd() && /^\d+$/.test(peekToken())) {
			...
			if (
				(instrToken.includes("8_lane") && (laneIndex < 0 || laneIndex > 15)) ||
