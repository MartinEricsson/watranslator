...
export function parseSIMDLaneBased(instrToken, position) {
	...
	if (instrToken === "v128.const") {
		...
		if (!atEnd()) {
			...
			while (!atEnd() && peekToken() !== ")" && immediates.length < maxValues) {
				if (/^-?(0x[0-9a-fA-F]+|\d+(\.\d+)?)$/.test(peekToken())) {
					...
					const value = isFloat
						...
						: is64Bit
							? parseSigned64BitHex(token)
							: parseDecimalOrHex(token);
			...
			if (immediates.length !== expectedValues) {
				throw createError(
					`v128.const with shape ${shape} requires exactly ${expectedValues} values, got ${immediates.length}`,
				);
			} // Generated by 🤖
	...
	if (instrToken === "i8x16.shuffle") {
		...
		while (!atEnd() && peekToken() !== ")" && laneIndices.length < 16) {
			if (/^[0-9]+$/.test(peekToken())) {
				// Parse lane index
				const index = Number.parseInt(getToken(), 10);

				// Validate lane index (must be 0-31 for i8x16.shuffle)
				if (index < 0 || index > 31) {
					throw createError(
						`i8x16.shuffle lane index must be 0-31, got ${index}`,
					);
				}
		...
		if (laneIndices.length !== 16) {
			throw createError(
				`i8x16.shuffle requires exactly 16 lane indices, got ${laneIndices.length}`,
			);
		}

		return {
			type: "i8x16.shuffle",
			opcode: "i8x16.shuffle",
			laneIndices,
			position,
		};
	} // Generated by 🤖
	...
	if (
		instrToken === "i8x16.extract_lane_u" ||
		instrToken === "i8x16.extract_lane_s" ||
		instrToken === "i16x8.extract_lane_s" ||
		instrToken === "i16x8.extract_lane_u" ||
		instrToken === "i32x4.extract_lane" ||
		instrToken === "i64x2.extract_lane" ||
		instrToken === "f32x4.extract_lane" ||
		instrToken === "f64x2.extract_lane"
	) {
		let laneIndex = 0;
		...
		if (!atEnd() && /^\d+$/.test(peekToken())) {
			...
			if (instrToken.startsWith("i8x16") && (laneIndex < 0 || laneIndex > 15)) {
				throw createError(
					`${instrToken} lane index must be 0-15, got ${laneIndex}`,
				);
			}

			if (instrToken.startsWith("i16x8") && (laneIndex < 0 || laneIndex > 7)) {
				throw createError(
					`${instrToken} lane index must be 0-7, got ${laneIndex}`,
				);
			}

			if (
				(instrToken.startsWith("i32x4") || instrToken.startsWith("f32x4")) &&
				(laneIndex < 0 || laneIndex > 3)
			) {
				throw createError(
					`${instrToken} lane index must be 0-3, got ${laneIndex}`,
				);
			}

			if (
				(instrToken.startsWith("i64x2") || instrToken.startsWith("f64x2")) &&
				(laneIndex < 0 || laneIndex > 1)
			) {
				throw createError(
					`${instrToken} lane index must be 0-1, got ${laneIndex}`,
				);
			}
		} // Generated by 🤖
