...
export function parseSIMDLaneBased(instrToken, position) {
        ...
        if (!atEnd()) {
            ...
            while (!atEnd() && peekToken() !== ')' && immediates.length < maxValues) {
                if (/^-?(0x[0-9a-fA-F]+|\d+(\.\d+)?)$/.test(peekToken())) {
                    ...
                    const value = isFloat ? Number.parseFloat(token) : is64Bit ? parseSigned64BitHex(token) : parseDecimalOrHex(token);
    ...
    if (instrToken === 'i8x16.extract_lane_u' || instrToken === 'i8x16.extract_lane_s' ||
        ...
        if (!atEnd() && /^\d+$/.test(peekToken())) {
            ...
            if ((instrToken.startsWith('i32x4') || instrToken.startsWith('f32x4')) && (laneIndex < 0 || laneIndex > 3)) {
                throw createError(`${instrToken} lane index must be 0-3, got ${laneIndex}`);
            }

            if ((instrToken.startsWith('i64x2') || instrToken.startsWith('f64x2')) && (laneIndex < 0 || laneIndex > 1)) {
    ...
    if (instrToken === 'i8x16.replace_lane' ||
        ...
        if (!atEnd() && /^\d+$/.test(peekToken())) {
            ...
            if ((instrToken.startsWith('i32x4') || instrToken.startsWith('f32x4')) &&
                (laneIndex < 0 || laneIndex > 3)) {
                throw createError(`${instrToken} lane index must be 0-3, got ${laneIndex}`);
            }

            if ((instrToken.startsWith('i64x2') || instrToken.startsWith('f64x2')) &&
