...
export function parseSIMDPlainOpcode(instructionTokenObj, loc) {
    // instructionTokenObj is the token object e.g. { value: "v128.and", line, column }
    ...
    if (plainOpCodes.includes(opValue)) {
        // Create and return the instruction object
        return { type: opValue, op: opValue, loc: loc }; // Added 'op' for consistency, though 'type' is often primary
    }

    // If not a plain opcode, return null (or false, but null is more consistent for "not parsed")
    return null;
}